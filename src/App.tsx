// ─────────────────────────────────────────────────────────────
// App.jsx
// Página principal de Memora — orquesta todas las secciones
// ─────────────────────────────────────────────────────────────
...
const globalCSS = `
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .nav-link {
    ...
    text-transform: uppercase; color: var(--chocolate); text-decoration: none; padding: 4px 0;
  }
  .nav-link::after {
  ...
  .chat-bubble-ai {
    ...
    box-shadow: 0 4px 16px rgba(90,15,36,0.1); font-size: 0.9rem; color: var(--chocolate);
    max-width: 260px; animation: fadeSlideUp 0.4s ease;
  }
  .chat-bubble-user {
    background: linear-gradient(135deg, var(--garnet), var(--crimson));
    border-radius: 18px 18px 4px 18px; padding: 12px 16px;
    box-shadow: 0 4px 16px rgba(142,27,58,0.3); font-size: 0.9rem; color: white;
    max-width: 220px; align-self: flex-end; animation: fadeSlideUp 0.4s ease;
  }
  .typing-dots span {
    display: inline-block; width: 6px; height: 6px; border-radius: 50%;
    background: var(--gray); margin: 0 2px; animation: blink 1.2s infinite;
  }
...
export default function App() {
  ...
  // ── Navegación por teclado: PgUp / PgDn / Home / End ──────
  useEffect(() => {
    const scrollTo = (idx) => {
  ...
  // ── Animación de chat (efecto tipeo) ──────────────────────
  useEffect(() => {
    let i = 0;
  ...
  useEffect(() => {
    const delays = { 1: 1500, 2: 1800, 3: 1600 };
    ...
    <>
      ...
      <div style={{ minHeight: "100vh", background: COLORS.beige, position: "relative" }}>
            ...
            <div style={{ display: "flex", justifyContent: "center" }}>
              <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "12px", paddingBottom: "20px", borderBottom: `1px solid ${COLORS.gold}20`, marginBottom: "20px" }}>
                  <div style={{ width: "44px", height: "44px", borderRadius: "50%", background: `linear-gradient(135deg, ${COLORS.garnet}, ${COLORS.bordeaux})`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "12px", minHeight: "220px" }}>
                  <div className="chat-bubble-ai">
                    {typedText}
                    <span style={{ animation: typedText === fullText ? "none" : "blink 0.7s infinite", marginLeft: "2px" }}>|</span>
                  </div>
                  {chatStep >= 1 && <div className="chat-bubble-user">Para mi pareja 💕 cumple 30 años</div>}
                  {chatStep >= 2 && <div className="chat-bubble-ai">¡Qué ocasión tan especial! ¿Qué le apasiona más: la aventura, el arte, la gastronomía o la naturaleza?</div>}
                  {chatStep >= 3 && <div className="chat-bubble-user">Le encanta la naturaleza y los viajes ✈️</div>}
                  {chatStep >= 4 && (
                ...
                <div style={{ marginTop: "16px", display: "flex", gap: "8px", background: `${COLORS.beige}80`, borderRadius: "100px", padding: "10px 10px 10px 20px", border: `1px solid ${COLORS.gold}30` }}>
                  ...
                  <button style={{ width: "34px", height: "34px", borderRadius: "50%", border: "none", background: `linear-gradient(135deg, ${COLORS.garnet}, ${COLORS.bordeaux})`, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <section id="como-funciona" style={{ padding: "72px 40px", background: COLORS.white, position: "relative", overflow: "hidden" }}>
          <div style={{ position: "absolute", top: "10%", right: "5%", width: "400px", height: "400px", borderRadius: "50%", background: `${COLORS.gold}08`, border: `1px solid ${COLORS.gold}15`, pointerEvents: "none" }} />
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "40px" }}>
              {steps.map((step, i) => (
                <FadeIn key={step.num} direction="up" delay={i * 150}>
                  <div className="card-hover gradient-border" style={{ background: "white", borderRadius: "24px", padding: "40px 32px", position: "relative", cursor: "default" }}>
                    <div style={{ position: "absolute", top: "24px", right: "24px", fontFamily: "'Playfair Display', serif", fontSize: "3.5rem", fontWeight: 900, color: `${COLORS.garnet}08`, lineHeight: 1 }}>{step.num}</div>
                    <div style={{ width: "56px", height: "56px", borderRadius: "16px", background: `linear-gradient(135deg, ${COLORS.garnet}15, ${COLORS.crimson}10)`, border: `1px solid ${COLORS.garnet}20`, display: "flex", alignItems: "center", justifyContent: "center", color: COLORS.garnet, marginBottom: "24px", padding: "14px" }}>{step.icon}</div>
          ...
          <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: "24px", alignItems: "stretch" }}>
              ...
              <FadeIn direction="left">
                <div className="card-hover" style={{ background: `linear-gradient(135deg, ${COLORS.bordeaux}, ${COLORS.garnet})`, borderRadius: "28px", padding: "36px 40px", position: "relative", overflow: "hidden", cursor: "default", height: "100%" }}>
                  <div style={{ position: "absolute", bottom: "-60px", right: "-60px", width: "300px", height: "300px", borderRadius: "50%", background: "rgba(255,255,255,0.05)" }} />
                  <div style={{ position: "absolute", top: "40px", right: "40px", width: "80px", height: "80px", borderRadius: "50%", background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)" }} />
                  ...
                  <div style={{ marginTop: "40px", display: "flex", gap: "12px", flexWrap: "wrap" }}>
                    {["Emocional", "Contextual", "Personalizado"].map((tag) => (
                      <span key={tag} style={{ background: "rgba(255,255,255,0.12)", border: "1px solid rgba(255,255,255,0.2)", color: "white", padding: "6px 14px", borderRadius: "100px", fontSize: "0.8rem", fontWeight: 500 }}>{tag}</span>
                    ))}
                  </div>
                </div>
              </FadeIn>

              {/* Beneficios apilados verticalmente */}
              <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                {features.slice(1).map((f, i) => (
                  <FadeIn key={f.title} direction="right" delay={i * 120}>
                    <div className="card-hover" style={{ background: "white", borderRadius: "20px", padding: "20px 24px", display: "flex", gap: "18px", alignItems: "center", cursor: "default", borderLeft: `3px solid ${COLORS.garnet}30` }}>
                      <div style={{ fontSize: "1.5rem", width: "48px", height: "48px", flexShrink: 0, background: `${COLORS.garnet}10`, borderRadius: "12px", display: "flex", alignItems: "center", justifyContent: "center" }}>{f.icon}</div>
                      <div>
          ...
          <div style={{ maxWidth: "900px", margin: "0 auto", textAlign: "center" }}>
            ...
            <div style={{ position: "relative", minHeight: "220px" }}>
              {testimonials.map((t, i) => (
                <div key={t.name} style={{ position: i === 0 ? "relative" : "absolute", top: 0, left: 0, right: 0, opacity: activeTestimonial === i ? 1 : 0, transform: activeTestimonial === i ? "translateY(0)" : "translateY(20px)", transition: "opacity 0.6s ease, transform 0.6s ease", pointerEvents: activeTestimonial === i ? "auto" : "none" }}>
                  <div style={{ background: `${COLORS.beige}60`, border: `1px solid ${COLORS.gold}30`, borderRadius: "24px", padding: "48px", maxWidth: "640px", margin: "0 auto" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "12px" }}>
                      <div style={{ width: "44px", height: "44px", borderRadius: "50%", background: `linear-gradient(135deg, ${COLORS.garnet}, ${COLORS.crimson})`, display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontWeight: 700 }}>{t.avatar}</div>
        ...
        {/* ════════════════════════════════════════════════════
            CTA
        ════════════════════════════════════════════════════ */}
        <section id="cta" style={{ padding: "120px 40px", margin: "0 40px 80px", background: `linear-gradient(135deg, ${COLORS.bordeaux} 0%, ${COLORS.garnet} 50%, ${COLORS.crimson} 100%)`, borderRadius: "40px", position: "relative", overflow: "hidden", textAlign: "center" }}>
          ...
          <FadeIn direction="up">
            <div style={{ position: "relative", zIndex: 2 }}>
              ...
              <p style={{ color: "rgba(255,255,255,0.8)", fontSize: "1.1rem", maxWidth: "450px", margin: "0 auto 44px", lineHeight: 1.7 }}>
                Únete a más de 2,400 personas que ya encontraron la manera de sorprender de verdad.
              </p>
              <div style={{ display: "flex", gap: "16px", justifyContent: "center", flexWrap: "wrap" }}>
                <button style={{ background: "white", color: COLORS.garnet, border: "none", padding: "18px 44px", borderRadius: "100px", fontSize: "1rem", fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", transition: "transform 0.3s ease, box-shadow 0.3s ease", boxShadow: "0 8px 32px rgba(0,0,0,0.2)" }}
                ...
                <button style={{ background: "transparent", color: "white", border: "1.5px solid rgba(255,255,255,0.4)", padding: "17px 32px", borderRadius: "100px", fontSize: "0.95rem", cursor: "pointer", fontFamily: "'DM Sans', sans-serif", fontWeight: 500, transition: "border-color 0.3s ease" }}
